...
export const Room = () => {
  ...
  const applyAudioPipeline = useCallback((rawStream, isMusicMode) => {
    ...
    console.log(`[AUDIO] ${isMusicMode ? "🎵 Music" : "🎙️ Voice"} mode applied`);
    return merged;
  }, []);
  ...
  const makePeer = useCallback((targetId, initiator, incomingSignal = null) => {
    ...
    const userEntry = {
      id: targetId, name: "User", isSelf: false,
      _onStream: null, _setStatus: null, _pendingStream: null,
    };
    ...
    const init = async () => {
      ...
      try {
        ...
        try {
          ...
          setError(`Mic/Camera blocked (${ae.name}). Allow karo aur reload karo.`);
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.grid}>
        <div style={styles.tile}>
          <video
            ...
            style={{ ...styles.video, display: camOn ? "block" : "none", transform: "scaleX(-1)" }}
          />
          {!camOn && (
            <div style={styles.avatarPlaceholder}>
              <span style={styles.avatarIcon}>🙋</span>
          ...
          <div style={styles.tileLabel}>
            <span>You</span>
            <span style={{ marginLeft: 8 }}>{micOn ? "🎙️" : "🔇"} {camOn ? "📹" : "📷"}</span>
          </div>
        </div>
      ...
      <div style={styles.controls}>
        <button onClick={toggleMic} style={{ ...styles.btn, background: micOn ? "#276749" : "#c53030" }}>
          {micOn ? "🎙️ Mute" : "🔇 Unmute"}
        </button>

        <button onClick={toggleCam} style={{ ...styles.btn, background: camOn ? "#276749" : "#c53030" }}>
          {camOn ? "📹 Cam Off" : "📷 Cam On"}
        </button>

        {/* Music / Voice mode switcher */}
        <button onClick={toggleMusicMode} style={{
          ...styles.btn,
          background: musicMode ? "#553c9a" : "#2c5282",
        ...
        <button onClick={leaveRoom} style={{ ...styles.btn, background: "#4a5568" }}>
          🚪 Leave
        </button>
      </div>
    </div>
...
const styles = {
  ...
  waiting: { textAlign: "center", color: "#718096", fontSize: 14, marginBottom: 20 },
  controls: { display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" },
  btn: {
