  ...
  return {
    ...
    destroy: () => {
      try {
        ...
        noiseGateNode?.disconnect(); compressor.disconnect();
        destination.disconnect(); ac.close();
      } catch {}
    },
  };
};
...
const ParticipantTile = React.memo(({ user, isLocal, camOn, micOn, videoRef: externalRef }) => {
  ...
  useEffect(() => {
    if (isLocal) { setHasVideo(camOn); return; }
    ...
    if (user._pendingStream) { user._onStream(user._pendingStream); user._pendingStream = null; }
    return () => { user._onStream = null; user._setStatus = null; };
...
export const Room = () => {
  ...
  const makePeer = useCallback((targetId, initiator, incomingSignal = null) => {
    ...
    const peer = new Peer({ initiator, trickle: true, stream: streamRef.current || undefined, config: ICE_CONFIG });
    const userEntry = { id: targetId, name: "User", isSelf: false, _onStream: null, _setStatus: null, _pendingStream: null };
    ...
    peer.on("error", (err) => { if (userEntry._setStatus) userEntry._setStatus("failed"); });
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        if (localVideoRef.current) { localVideoRef.current.srcObject = raw; localVideoRef.current.play().catch(() => {}); }
      } catch (err) {
        try {
          const audioRaw = await navigator.mediaDevices.getUserMedia({ audio: { echoCancellation: true, noiseSuppression: false, autoGainControl: false }, video: false });
          if (!disposed) { rawStreamRef.current = audioRaw; applyAudioPipeline(audioRaw, false); setMicOn(true); setError("Camera unavailable — audio only."); }
          else { audioRaw.getTracks().forEach((t) => t.stop()); return; }
        } catch (ae) {
          setError(`Mic/Camera blocked (${ae.name}). Please allow access and reload.`);
          streamRef.current = null;
        }
      }
      if (disposed) return;

      socket.off("all-users"); socket.off("user-joined"); socket.off("receiving-signal"); socket.off("answer-signal"); socket.off("user-left");
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-950 text-white overflow-hidden" style={{ fontFamily: "'Google Sans', 'Roboto', sans-serif" }}>
      ...
      <header className="flex items-center justify-between px-5 py-3 bg-gray-900 border-b border-white/10 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-gray-400 font-mono bg-gray-800 px-2 py-1 rounded-md">{id}</span>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* ── Left: Main video / screen share ── */}
        <div className="flex-1 flex flex-col overflow-hidden p-3 gap-3">

          {/* Screen share or normal grid */}
          {isScreenMode ? (
            /* Screen share is the main focus */
            <div className="flex-1 relative rounded-2xl overflow-hidden bg-gray-900 border border-white/10 flex items-center justify-center">
              <video
                ref={screenVideoRef}
                autoPlay
                playsInline
            ...
            <div className={`flex-1 grid gap-3 content-start ${
              ...
              users.length === 2 ? "grid-cols-1 md:grid-cols-2" :
              users.length <= 4 ? "grid-cols-2" :
              "grid-cols-2 md:grid-cols-3"
            }`}>
              {/* Local tile */}
              <ParticipantTile
                user={{ id: "self", name: "You" }}
                isLocal
                camOn={camOn}
                micOn={micOn}
          ...
          {/* Waiting message */}
          {ready && remoteUsers.length === 0 && !isScreenMode && (
        ...
        {/* ── Right Sidebar: participant strip (only visible during screen share OR when there are remote users) ── */}
        {(isScreenMode || (remoteUsers.length > 0)) && isScreenMode && (
          <aside className="w-56 flex flex-col gap-2 p-3 bg-gray-900/60 border-l border-white/10 overflow-y-auto shrink-0">
            <p className="text-xs text-gray-500 font-medium px-1 mb-1">Participants</p>

            {/* Local self tile in sidebar */}
            <ParticipantTile
              user={{ id: "self", name: "You" }}
              isLocal
              camOn={camOn}
            ...
            {remoteUsers.length === 0 && (
              <div className="text-center py-4 text-gray-600 text-xs">No others yet</div>
            )}
          </aside>
        )}
      </div>
      ...
      <div className="shrink-0 bg-gray-900 border-t border-white/10 px-6 py-4 flex items-center justify-center gap-3 flex-wrap">
        ...
        <button
          ...
          title={musicMode ? "Switch to Voice Mode" : "Switch to Music Mode"}
          className={`flex items-center gap-2 px-4 py-3 rounded-full font-medium text-sm transition-all duration-200 ${
            musicMode
